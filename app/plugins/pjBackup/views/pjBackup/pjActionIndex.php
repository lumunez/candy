...
if (isset($tpl['status']))
{
	...
	pjUtil::printNotice($titles['AB01'], $bodies['AB01']);
	if (isset($_GET['err']))
	{
	...
	<form action="<?php echo PJ_INSTALL_URL; ?>index.php?controller=pjBackup&amp;action=pjActionIndex" method="post" class="pj-form form">
		...
		<fieldset class="fieldset white overflow">
			<legend><?php __('menuBackup'); ?></legend>
			<div class="float_left w200">
				<p>
					<label class="title"><?php __('lblBackupDatabase'); ?></label>
					<span class="left">
						<input type="checkbox" name="db" value="1" checked="checked" />
					</span>
				</p>
				<p>
					<label class="title"><?php __('lblBackupFiles'); ?></label>
					<span class="left">
			...
			<p class="float_left">
				<input type="submit" value="<?php __('btnBackup'); ?>" class="pj-button" />
			</p>
		</fieldset>
	</form>
