...
(function ($, undefined) {
	...
	Datagrid.prototype = {
		_attachDatagrid: function (target, settings) {
			...
			$target.on("mouseenter", ".pj-table tbody tr", function () {
			...
			}).on("change", ".pj-table-toggle-rows", function () {
				var $this = $(this);
				if ($this.is(":checked")) {
					$(".pj-table-select-row").prop("checked", true).closest("tr").addClass("pj-table-row-active");
				} else {
					$(".pj-table-select-row").prop("checked", false).closest("tr").removeClass("pj-table-row-active");
				}
				...
				return FALSE;
			}).on("click", ".pj-table-icon-delete", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				...
				return FALSE;
			}).on("click", ".pj-paginator-list-paginate", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				...
				return FALSE;
			}).on("click", ".pj-paginator-list-prev, .pj-paginator-list-next", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				var $this = $(this),
				...
				self._loadDatagrid.call(self, target, inst.settings.dataUrl, inst.settings.content.column, inst.settings.content.direction, p, inst.settings.content.rowCount);
				return FALSE;
			}).on("click", ".pj-paginator-button-actions, .pj-paginator-row-count", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				var diff, lf,
				...
				$(".pj-menu-list-wrap").not($list).hide();
				return FALSE;
			}).on("click", ".pj-selector-row-count", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				...
				return FALSE;
			}).on("keypress", ".pj-selector-goto", function (e) {
			...
			}).on("keypress", ".pj-table-cell-editable > .pj-selector-editable", function (e) {
				switch (e.keyCode ? e.keyCode : e.which) {
					case 13: //Enter
						$(this).trigger("save");
						break;
				}
			}).on("keyup", ".pj-table-cell-editable > .pj-selector-editable", function (e) {
				switch (e.keyCode ? e.keyCode : e.which) {
					case 27: //Escape
						$(this).hide().siblings(".pj-table-cell-label").show();
						break;
				}
			}).on("save", ".pj-selector-editable", function (e) {
				var $this = $(this),
			...
			}).on("click", ".pj-paginator-action", function (e) {
				var i, $form,
					$this = $(this),
			...
			$.get(inst.settings.dataUrl).done(function (data) {
		...
		_renderDatagrid: function (target) {
			...
			for (i = 0, iCnt = inst.settings.columns.length; i < iCnt; i++) {
				$th = $("<th>");
				if (inst.settings.columns[i].sortable) {
					$("<div>").addClass("pj-table-sort-label").text(inst.settings.columns[i].text).appendTo($th);
					upClass = "";
					downClass = "";
					...
					$th.text(inst.settings.columns[i].text);
				}
						...
						if (inst.settings.buttons[j].type == "menu" && inst.settings.buttons[j].items !== undefined) {
							...
							for (k = 0, kCnt = inst.settings.buttons[j].items.length; k < kCnt; k++) {
								...
								if (inst.settings.buttons[j].items[k].ajax === true) {
									$tmp.bind("click", function (render) {
										return function (e) {
											if (e && e.preventDefault) {
												e.preventDefault();
											}
			...
			if (inst.settings.paginator) {
				...
				if (inst.settings.paginator.actions && inst.settings.paginator.actions.length > 0) {
					$a = $("<a>", {
						"href": "#"
					}).addClass("pj-button pj-paginator-button-actions").text(self.messages.choose_action);
				...
				if (inst.settings.paginator.paginate) {
					...
					for (i = ii; i <= ic; i++) {
						$a = $("<a>", {
							...
							"data-page": i
						}).addClass("pj-paginator-list-paginate").text(i);
				...
				if (inst.settings.paginator.rowCount) {
					$("<a>", {
						"href": "#"
					}).addClass("pj-button pj-paginator-row-count").text(inst.settings.content.rowCount).appendTo($span);
					...
					$("<li>").append($("<span>", {
						css: {"fontSize": "11px"}
					}).text(self.messages.items_per_page)).appendTo($ul);
					for (i = 10; i < 60; i += 10) {
						$li = $("<li>").append( $("<a>", {
							"href": "#",
							"data-rowcount": i
