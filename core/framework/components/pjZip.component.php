 ...
 * @example
	...
	$ZipComponent=new ZipComponent();

	// Code to Zip a single file
	//$ZipComponent->addDirectory($outputDir);
	//$fileContents=file_get_contents($fileToZip);
	//$ZipComponent->addFile($fileContents, $outputDir.$fileToZip);

	//Code toZip a directory and all its files/subdirectories
	$ZipComponent->zipDirectory($directoryToZip,$outputDir);

	$rand=md5(microtime().rand(0,999999));
	$zipName=$rand."_".$zipName;
	$fd=fopen($zipName, "wb");
	$out=fwrite($fd,$ZipComponent->getZippedfile());
	fclose($fd);
	$ZipComponent->forceDownload($zipName);
	@unlink($zipName);
 */
class pjZip
{
